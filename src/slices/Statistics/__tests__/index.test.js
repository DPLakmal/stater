...
describe("Statistics Component", () => {
  ...
  //divider
  it("third variation with divider renders correctly", () => {
    const data = simplifyMockData(mocks[2]);
    data.primary.show_divider = true;
    const { container } = render(<Component slice={data} />);
    expect(container).toMatchSnapshot();
  });
  it("third variation without divider renders correctly", () => {
    const data = simplifyMockData(mocks[2]);
    data.primary.show_divider = false;
    const { container } = render(<Component slice={data} />);
    expect(container).toMatchSnapshot();
  });
});
